...
const CurrentRequestDock = (
    { creator, description, date } : 
    {creator: string | null, description: string | null, date: string | null}) => {
...
const MyItemDisplay = ({apiKey} : {apiKey: string}) => {
    ...
    return (
        <div className='flex flex-row gap-4 text-gtGold w-screen h-screen items-center justify-center'>
            ...
            <div 
            className='flex flex-col gap-2 rounded-lg items-center w-1/4 h-3/4'>
                {requests?
                ...
                (requests?.map((request) => (
                    ...
                    className='flex flex-row group w-full h-28 justify-between border-[1px] p-4 overflow-scroll gap-4 duration-300 cursor-pointer bg-mainHover hover:bg-mainHover2 border-gray-500 rounded-lg'>
                        <div className='flex flex-col items-start gap-1 w-[60%]'>
                            ...
                            <p className='text-xs text-start h-12 overflow-clip text-gray-400'>{request.description}</p>
                        </div>
                        <div className='flex flex-col justify-between items-end'>
                            ...
                            <p className='text-xs text-gray-400'>{request.created_at.substring(0, 10)}</p>
                        </div>
                    </button>
                )))
                ...
                <div 
                className='flex flex-row group w-full h-28 justify-between border-[1px] bg-mainTheme border-gray-500 rounded-lg'>
