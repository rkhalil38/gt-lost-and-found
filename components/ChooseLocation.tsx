...
const ChooseLocation = ({apiKey, setToggled, setLocation} : {apiKey: string, setToggled: Function, setLocation: Function}) => {
    ...
    useEffect(() => {
        ...
        const initMap = async () => {
          ...
          const marker = new AdvancedMarkerElement({
            position: new google.maps.LatLng(defaultProps.center.lat, defaultProps.center.lng),
            map: map,
            content: pinSvg,
            gmpDraggable: true,
          })
          ...
          marker.addListener('dragend', () => {setLocation({...location, lat: marker.position?.lat, lng: marker.position?.lng})})

          map.addListener('click', (click: google.maps.MapMouseEvent) => {
            setLocation({lat: click.latLng?.lat() as number, lng: click.latLng?.lng() as number})
    ...
    return (
      <div className='flex rounded-lg w-full h-full animate-in'>
          <h1 className='absolute animate-pulse text-sm text-white top-2 left-2 z-10'>Click on or drag the pin to where you found the item.</h1>
          <div ref={mapRef} className='w-full h-full'/>
          <button onClick={() => setToggled(false)} className='flex absolute bottom-4 right-4 w-36 h-10 text-xs justify-center items-center rounded-lg border-[1px] border-gray-400 bg-mainTheme hover:bg-gtGold duration-300'>
